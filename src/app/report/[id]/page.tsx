...
import ReadableContextSection from "@/src/components/report/ReadableContextSection";
...
export default async function ShareableReportPage({ params }: PageProps) {
  ...
  const riskyFiles = getRiskyFiles(report, 7);
    ...
    <main className="page-shell">
      <div className="page-grid">
        ...
        <Reveal className="surface px-6 py-7 sm:px-8" delay={0.26}>
          <span className="window-ornament" aria-hidden="true" />
          <ReadableContextSection
            context={readableContext}
            riskyFilePaths={riskyFiles.map((file) => file.filePath)}
          />
        </Reveal>
